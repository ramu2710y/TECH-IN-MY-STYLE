/* ═══════════════════════════════════════════════════════════════════
   Perfect JS Topics Generator - High Quality Examples
   Creates topic-specific, educational examples for all 111 topics
═══════════════════════════════════════════════════════════════════ */
...
// Topic-specific examples with perfect code
const topicExamples = {
  // ═══════════════════════════════════════════════════════════════
  // GETTING STARTED
  // ═══════════════════════════════════════════════════════════════
...
const jsTopicExamples = [\n\n`;
  ...
  // Add the 5 completed examples
  for (const [topic, data] of Object.entries(topicExamples)) {
    content += `  {\n`;
    content += `    topic: "${topic}",\n`;
    content += `    description: "${data.desc}",\n`;
    content += `    htmlCode: \`${esc(data.html)}\`,\n`;
    content += `    cssCode: \`${esc(data.css)}\`,\n`;
    content += `    jsCode: \`${esc(data.js)}\`,\n`;
    content += `  },\n\n`;
  }

  content += `];\n\n`;
  content += `export function getTopicExample(topic) {\n`;
  content += `  return jsTopicExamples.find((ex) => ex.topic === topic);\n`;
  content += `}\n\n`;
  content += `export default jsTopicExamples;\n`;
...
const outputPath = path.join(__dirname, 'client', 'src', 'modules', 'js-basic-course', 'data', 'jsTopicExamples_PERFECT_START.jsx');
fs.writeFileSync(outputPath, content, 'utf8');

console.log('✅ Generated 5 perfect examples as a starting point!');
console.log('📁 File:', outputPath);
console.log('📊 Size:', (content.length / 1024).toFixed(2), 'KB');
console.log('\n🎯 These examples demonstrate:');
console.log('   - Topic-specific HTML/CSS/JS');
console.log('   - Perfect code formatting');
console.log('   - Educational demonstrations');
console.log('   - Beautiful UI designs');
console.log('   - Console logging');
console.log('\n📝 Next: Review these 5 examples, then I\'ll generate all 111!');
